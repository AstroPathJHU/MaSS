function [infm] = getinformcytovals(fils, imnum)
    %    
    Markers.all = {'PDL1','CD8','FoxP3','Tumor','PD1','CD163'};
    Markers.Opals = {'520','540','570','620','650','690'};
    %
    f1 = cellfun(@(x)['TotalCytoplasm',x],Markers.Opals,'Uni',0);
    f2 = cellfun(@(x)['MeanCytoplasm',x],Markers.Opals,'Uni',0);
    infm.cytovals = fils(:,[{'CellID','CytoplasmArea'},f1,f2]);
    %
    for i1 = 1:length(Markers.Opals)
       a = infm.cytovals(:,f1{i1});
       a = table2array(a);
       b = infm.cytovals(:,'CytoplasmArea');
       b = table2array(b);
       c = double(a)./double(b);
       %
       infm.cytovals.(['ExpectedMeanCytoplasm',Markers.Opals{i1}]) = c(:);
    end
    %
    % create a figure
    %
    figname =  ['Mean Cytoplasm InForm Evaluation of Image ', imnum];
    ...
    for i1 = 1:length(Markers.Opals)
        op = Markers.Opals{i1};
        x = infm.cytovals.(['ExpectedMeanCytoplasm', op]);
        ...
        xlabel({['Expected Mean Cytoplasm ', op],...
            'Calculated by: Inform cyto Int / Inform Total Area'})
