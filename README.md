...
#### <div align="center">Created by: Benjamin Green<sup>1</sup>, Sneha Berry<sup>2</sup>, Logan Engle<sup>2</sup>, Nicolas Giraldo<sup>3</sup>, Peter Nguyen<sup>2</sup>, Tricia Cottrell<sup>3</sup>, Janis Taube1<sup>2,3</sup>, and Alex Szalay<sup>4</sup></div>
...
Finally, the code produces a ```*\Results\cells.csv``` file. This can be used to ensure that the correct markers have corresponding cell counts. As the code does not pre-determine which markers should match, this is used as a quality check for matching segmentations. 
 - The ```Image``` column identifies the HPF. 
 - The ```Marker``` columns provide the total number of cells generated by the segmentation for each marker. 
 - The ```Clusters``` column combines all markers with the same number of cells.
